...
var obuSetSleepTime = function(time, deviceId, callback) {
  sendData(DataUtil.makeCCSendData(time[0]), deviceId, (code, data) => {
    if (code == 0) {
...
var get0016Info = function(bleDeviceId, callback) {
  ...
  let random=""                   //随机数
  sendApdu(1, ["00A40000023f00", "0084000008", "00B0960037"], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000" && data[2].slice(-4) == '9000') {
        random = data[1].slice(0,-4)
        cardholderID = data[2].slice(0, 2)
        staffID = data[2].slice(2, 4)
        cardholderName = data[2].slice(4, 44)
        cardNumber = data[2].slice(44, 108)
        cardType = data[2].slice(108, 110)
        switch (cardType) {
          case "16":
            cardType = "储值卡";
            break;
...
var set0016Info = function(cardInfo, bleDeviceId, callback) {
  command = "00D69600" + num2hex(parseInt(cardInfo.length / 2), 1, true);
  sendApdu(1, ["00A40000023f00",  command], bleDeviceId, (code,data)=>{
...
var get0015Info = function (bleDeviceId, callback) {
  ...
  let random=""               //随机数
  sendApdu(1, ["00A40000021001", "0084000008", "00B095002b"], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000" && data[2].slice(-4) == '9000') {
       random = data[1].slice(0, -4)
        let cardInfo = data[2].slice(0, -4)
...
var set0015Info = function (cardInfo, bleDeviceId, callback) {
  command = "00D69500" + num2hex(cardInfo,parseInt(cardInfo.length / 2),  true);
  sendApdu(1, ["00A40000021001", command], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000") {
      callback(0, data)
    } else {
      callback(1, data)
 ...
 * 圈存
 */
var loadCard = function (mac2, bleDeviceId, callback) {
  if (mac2.length != 22) {
  ...
  sendApdu(1, [mac2Cos], bleDeviceId,(code, data) => {
    if (code == 0) {
 ...
 * 写金额
 */
var writeMoney = function (comMoney, bleDeviceId,callback){
  sendApdu(1, ["00A40000021001", num2hex(comMoney, parseInt(comMoney.length / 2), true)], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000") {
      callback(0, data)
    } else {
      callback(1, data)
...
var getVehiclePara = function (bleDeviceId, callback){
  ...
  sendApdu(2, ["00A40000023F00","0084000008","00B0810B08"], bleDeviceId, (code, data) => {
    if (code == 0 && data[2].slice(-4) == "9000") {
      random=data[1].slice(0,-4)
      contractId=data[2].slice(0,-4)
      callback(0, {
        random:random,
...
var writeVehicle = function (commandVeh,bleDeviceId,callback){
  sendApdu(2, ["00A4000002DF01", commandVeh], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000") {
      callback(0, data)
    } else {
      callback(1, data)
...
var getSysPara = function (bleDeviceId, callback){
  ...
  let expiredDate = ""       //到期时间
  let disStatus=""          //拆卸状态
  sendApdu(2, ["00A40000023F00", "0084000008", "00B081001B"], bleDeviceId, (code, data) => {
    if (code == 0 && data[2].slice(-4) == "9000") {
      random = data[1].slice(0, -4)
      let info = data[2].slice(0, -4)
...
var writeSys = function (commandSys, bleDeviceId, callback) {
  sendApdu(2, ["00A40000023F00", commandSys], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4) == "9000") {
      callback(0, data)
    } else {
      callback(1, data)
