...
var initDevice = function(bleDeviceId, callback) {
  sendData(DataUtil.makeC0SendData(), bleDeviceId, (code, data) => {
=======
var initDevice = function (callback) {
  sendData(DataUtil.make80SendData(), (code, data) => {
>>>>>>> e772ff189dc63b1e9499cabdabf939c5eed3b1b7
    if (code == 0) {
...
/**
 * 对 B-OBU 设备操作
 * instruction : 取设备序列号：02，取设备电量：03
 */
var deviceChannel = function(instruction, callback) {
  sendData(DataUtil.makeC5SendData(DataUtil.makeDBMTLV(instruction)), (code, data) => {
    if (code == 0) {
...
/**
 * OBU apdu指令透传
 * type : 0 - IC卡， 1 - ESAM， 2-display（指令只支持单条，如闪前灯3次：8103000203），3-beeper（指令只支持单条，蜂鸣3次：81020203）
 * apdu : apdu 指令数组
 */
var sendApdu = function(type, apdu, callback) {
  ...
  if (type == 0 || type == 1) {
    ...
    tlv = DataUtil.makeDBMTLV(apdu[0])
    dataType = "03"
    if (type == 3) {
      dataType = "04"
    }
  ...
  sendData(DataUtil.makeC2SendData(dataType, tlv), (code, data) => {
    if (code == 0) {
      if (type == 0 || type == 1) {
...
/**
 * 发送apdu指令，
 * type : 0 - IC卡， 1 - ESAM， 2-display（指令只支持单条，如闪前灯3次：8103000203），3-beeper（指令只支持单条，如蜂鸣3次：81020203）
...
var upNewPipe = function(state, callback) {
  deviceChannel("0a01" + DataUtil.num2hex(state, 1, true), (code, data) => {
    typeof callback == FUNCTION && callback(code, data)
  })
}
