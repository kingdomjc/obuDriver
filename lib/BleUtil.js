 ...
 * 关闭蓝牙适配器
 */
var closeBle = function (_bleDeviceId,callback) {
  if (_bleDeviceId!=""&&connectionState){
    disconnectBle(_bleDeviceId,function (code) {
      if (code == 0) {
        console.log("关闭连接成功")
 ...
 * 断开蓝牙连接
 */
var disconnectBle = function (_bleDeviceId,callback) {
...
var getCardBalance = function (bleDeviceId,callback) {
  sendApdu(1, ["00A40000021001", "805C000204"], bleDeviceId, (code, data) => {
    if (code == 0 && data[1].slice(-4)=="9000") {
      ...
      typeof callback == FUNCTION && callback(0, parseInt(loadMac1Info.slice(0, 8), 16))
    } else {
      typeof callback == FUNCTION && callback(code, data)
    }
  })
...
var loadCardInit = function (credit, terminnalNo, pinCode, procType, keyIndex, bleDeviceId, callback) {
  ...
  let macCos = "805000" + procType + "0B" + keyIndex + DataUtil.num2hex(credit, 4, true) + DataUtil.num2hex(terminnalNo,6,true)+"10"
  sendApdu(1, ["00A40000021001", pinCos, "00B095002b", macCos], bleDeviceId, (code, data) => {
    if (code == 0) {
      let loadMac1Info = data[3].slice(0, -4)
      let balance = ""+parseInt(loadMac1Info.slice(0, 8),16)
      let trade_no = loadMac1Info.slice(8, 12)
      let rand = loadMac1Info.slice(16, 24)
      let mac1 = loadMac1Info.slice(24, 32)
      let cardId = data[2].slice(20, 40)
      typeof callback == FUNCTION && callback(0, {
        balance: balance,
        serial: trade_no,
        icRandom: rand,
...
var getVehiclePara = function (bleDeviceId, callback){
  ...
  sendApdu(1, ["00A40000021001", "00B095002b"], bleDeviceId, (code, data1) => {
    if (code == 0 && data1[1].slice(-4) == "9000") {
      ...
      sendApdu(2, ["00A40000023f00", "00B081001B", "00A4000002df01","0084000004"], bleDeviceId, (code, data) => {
        if (code == 0 && data[2].slice(-4) == "9000") {
          random = data[3].slice(0, -4)
          contractId = data[1].slice(20, 36)
          callback(0, {
            random: random,
