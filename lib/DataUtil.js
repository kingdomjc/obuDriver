/**
 * 16进制数字转换成指定字节数的（16进制字符串）hexString
 * num：转换的number值
 * bitNum：转换后的字节数
 ...
 * hexStr:16进制字符串
 */
var hex2Ascll = function(hexStr) {
  let str = ""
  for (let i = 0; i < hexStr.length; i += 2) {
    str = str + String.fromCharCode(parseInt(hexStr.slice(i, i + 2), 16))
  }
  return str
}
/**
...
function makeCCSendData(times) {
  let data = "81"
  let len = num2hex(parseInt(("c6" + times).length / 2), 2, false)
  data += hexTimes
  return makeFrame(data)
}
